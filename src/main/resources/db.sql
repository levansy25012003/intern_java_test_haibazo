...
SELECT *
FROM product
JOIN product_color pc on product.id = pc.product_id
JOIN color cl on pc.color_id = cl.color_id
JOIN style s on product.style_id = s.style_id
JOIN category c on product.category_id = c.category_id
WHERE cl.color_name = 'red' AND s.style_name = 'M' AND c.category_id = 1 AND s.style_id = 1;
